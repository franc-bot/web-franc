...
async function sendDeveloperReport() {
  ...
  const reportMessage = [
    `Halo Franc Developer ོ, saya ingin mengirim ${typeLabel.toLowerCase()}.`,
    "",
...
// =========================================================
const maxielThemeKey = "franc_theme"
const maxielColorsKey = "franc_dashboard_colors"
...
// =========================================================
const webIdentityNameKey = "franc_web_identity_name"
const webIdentityIconKey = "franc_web_identity_icon"
const webIdentityInstallKey = "franc_web_identity_install"
const defaultWebIdentityName = "Franc"
...
function getStoredAppIconSet() {
  try {
    const raw = localStorage.getItem("franc_web_identity_icons")
...
function applyWebIdentityText(name) {
  ...
  nodes.forEach(node => {
    ...
    if (node.nodeValue.includes("FRANC")) {
      node.nodeValue = node.nodeValue.replaceAll("FRANC", upperName)
    }
  })
}
...
function updateAppIdentityUI() {
  ...
  // bukan manifest.json default yang ada di server.
...
saveAppIdentity?.addEventListener("click", async () => {
  ...
  localStorage.setItem("franc_web_identity_icons", JSON.stringify(icons))
...
resetAppIdentity?.addEventListener("click", () => {
  ...
  localStorage.removeItem("franc_web_identity_icons")
...
async function sendAIMessage(){
  ...
  try{
    ...
    console.error("Franc AI:",error)
    addAIMessage("Maaf, Franc AI sedang mengalami masalah. Coba lagi beberapa saat lagi.","bot")
  }finally{
